<template>
  <div :class="styles.categorization.root">
    <div :class="[isStepper ? styles.categorization.stepper : styles.categorization.category]">
      ...
      >
        ...
        >
          ...
          >
            <span v-if="isStepper" :class="styles.categorization.stepperBadge">{{ index+1 }}</span>

            <label>{{ item.label }}</label>
          </button>
        </div>

        <hr v-if="isStepper && index!==categories.length-1" :class="styles.categorization.stepperLine" />
    ...
    <footer v-if="showNavButtons" :class="styles.categorization.stepperFooter">
      ...
      <div @click="selected=selected-1" v-if="selected>0" :class="styles.categorization.stepperButtonNext">
        <button :disabled="!categories[selected-1].isEnabled">
          {{ t('categorizationStepperBack', 'back') }}
        </button>
      </div>

      <div @click="selected=selected+1" v-if="selected+1<=categories.length-1" :class="styles.categorization.stepperButtonNext">
        <button :disabled="!categories[selected+1].isEnabled">
...
const layoutRenderer = defineComponent({
  ...
  setup(props: RendererProps<Layout>) {
    ...
    const showNavButtons = isStepper && !!input.appliedOptions?.value.showNavButtons
