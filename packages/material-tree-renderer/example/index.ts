...
const detailSchemata: {
  ...
  {
    tester: schema =>
      _.keys(schema.properties).some(key => key === 'done')
        ? 1
        : NOT_APPLICABLE,
    uischema: taskView
  },
  {
    tester: schema =>
      _.keys(schema.properties).some(key => key === 'birthday')
        ? 1
        : NOT_APPLICABLE,
    uischema: userView
  },
  {
    tester: schema =>
      _.keys(schema.properties).some(key => key === 'users')
        ? 1
        : NOT_APPLICABLE,
...
const filterPredicate = (data: any) => (property: Property): boolean => {
  if (
    _.has(data, '_type') &&
    _.has(property.schema, 'properties._type.default')
  ) {
