...
var ValidationService = (function () {
    ...
    ValidationService.prototype.validate = function (instance, schema) {
        ...
        this.checkObjects = [];
        this.clear(instance);
        var results = validator.validateMultiple(instance, schema);
        results['errors'].forEach(function (error) {
            if (error['schemaPath'].indexOf('required') !== -1) {
                var propName = error['dataPath'] + '/' + error['params']['key'];
                _this.validationResults.get(instance)[propName] = pathutil_1.PathUtil.beautifiedLastFragment(error['params']['key']) + " is a required field";
            }
            else {
