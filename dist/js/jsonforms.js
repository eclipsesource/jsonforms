/*! jsonforms - v0.0.15 - 2016-02-25 Copyright (c) EclipseSource Muenchen GmbH and others. */ 
'use strict';
...
(function (JSONForms) {
    var RenderService = (function () {
        ...
        RenderService.prototype.render = function (scope, element, services) {
            ...
            if (element['scope']) {
                ...
                schemaPath = JSONForms.PathUtil.filterIndexes(indexedSchemaPath);
    ...
    var ContainerRenderDescription = (function () {
        function ContainerRenderDescription(size, elements, template, services, element) {
            ...
            this.path = JSONForms.PathUtil.normalize(element['scope'] ? element['scope']['$ref'] : "");
          ...
          this.link = function (scope, element) {
              if (scope.element.templateUrl) {
                  _this.$templateRequest(scope.element.templateUrl).then(function (template) {
                      var updatedTemplate = _this.replaceJSONFormsAttributeInTemplate(template, scope.element.path);
                      var compiledTemplate = _this.$compile(updatedTemplate)(scope);
                      element.replaceWith(compiledTemplate);
                  });
              }
              else {
                  var updatedTemplate = _this.replaceJSONFormsAttributeInTemplate(scope.element.template, scope.element.path);
                  var template = _this.$compile(updatedTemplate)(scope);
                  element.replaceWith(template);
              }
          };
      }
      DynamicWidget.prototype.replaceJSONFormsAttributeInTemplate = function (template, propertyPath) {
...
//--------------------------------------------------------------------------------------------------------------------
  ...
  var ArrayRenderer = (function () {
      function ArrayRenderer(renderService, pathResolver) {
          this.renderService = renderService;
          this.pathResolver = pathResolver;
          this.maxSize = 99;
          this.priority = 99;
      }
      ArrayRenderer.createGroup = function (elements) {
          return {
              "type": "Group",
              "elements": elements
          };
      };
      ArrayRenderer.createControl = function (schemaPath, prop) {
          return {
              "type": "Control",
      ...
      ArrayRenderer.prototype.createControlGroupPerItem = function (schemaPath, items, dataLength) {
          return _.range(0, dataLength).map(function (index) {
              var elements = _.keys(items['properties']).map(function (key) {
                  return ArrayRenderer.createControl(schemaPath + "/items/" + index + "/properties/" + key, key);
              });
              return ArrayRenderer.createGroup(elements);
          });
      };
      ArrayRenderer.prototype.createControlsForSubmit = function (items, schemaPath, submitElement, services) {
          var unboundControls = _.keys(items['properties']).map(function (prop) {
              return ArrayRenderer.createControl(schemaPath + "/items/properties/" + prop, prop);
          });
          var renderDescriptionsForSubmit = JSONForms.RenderDescriptionFactory.renderElements(unboundControls, this.renderService, services);
          return renderDescriptionsForSubmit.map(function (renderDescription) {
      ...
      ArrayRenderer.prototype.generateControlDescriptions = function (items, schemaPath, currentDescriptions, dataSize, services) {
          if (currentDescriptions.length === dataSize) {
              return currentDescriptions;
          }
          currentDescriptions.splice(0, currentDescriptions.length);
          var controlGroups = this.createControlGroupPerItem(schemaPath, items, dataSize);
          var renderDescriptions = JSONForms.RenderDescriptionFactory.renderElements(controlGroups, this.renderService, services);
          return renderDescriptions.reduce(function (descriptions, renderDescription) {
              var foundRenderDesc = _.find(descriptions, function (desc) { return _.isEqual(renderDescription, desc); });
      ...
      ArrayRenderer.prototype.render = function (element, subSchema, schemaPath, services) {
          var _this = this;
          var data = services.get(JSONForms.ServiceId.DataProvider).getData();
          if (!Array.isArray(data)) {
              data = this.pathResolver.resolveInstance(data, schemaPath);
          }
          var resolvedSubSchema = this.pathResolver.resolveSchema(subSchema, schemaPath);
          var items = resolvedSubSchema.items;
          var label = element.label ? element.label : JSONForms.PathUtil.beautifiedLastFragment(schemaPath);
          if (element.options != undefined && element.options['simple']) {
              var controlDescription = JSONForms.RenderDescriptionFactory.createControlDescription(schemaPath, services, element);
              var properties = _.keys(items['properties']);
              ...
              var submitElement = {};
              var supportsSubmit = element.options != undefined && element.options['submit'];
              var generatedGroups = this.createControlGroupPerItem(schemaPath, items, data.length);
              var buttonText = JSONForms.PathUtil.beautifiedLastFragment(schemaPath);
              ...
              var containeeDescriptions = JSONForms.RenderDescriptionFactory.renderElements(generatedGroups, this.renderService, services);
              var containerDescription = JSONForms.RenderDescriptionFactory.createContainerDescription(this.maxSize, containeeDescriptions, template, services, element);
              if (supportsSubmit) {
                  ...
                  containerDescription['submitCallback'] = function () { return data.push(_.clone(submitElement)); };
              }
              containerDescription['generateControlDescriptions'] = function (data) {
                  return _this.generateControlDescriptions(items, schemaPath, containerDescription['elements'], data.length, services);
              };
...
// Optimized: Yes
// File:      components/ng-services/capitalize/capitalize.filter.js
//====================================================================================================================
...
}) (angular.module ('jsonforms.filters.capitalize', []));
