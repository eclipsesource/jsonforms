...
var HorizontalController = (function (_super) {
    ...
    HorizontalController.prototype.updateChildrenLabel = function (elements) {
        var labelExists = elements.reduce(function (atLeastOneLabel, element) {
            return atLeastOneLabel || Labels_1.LabelObjectUtil.shouldShowLabel(element.label);
        }, false);
        if (labelExists) {
            elements.forEach(function (element) {
                var showElementLabel = Labels_1.LabelObjectUtil.shouldShowLabel(element.label);
                if (!showElementLabel) {
