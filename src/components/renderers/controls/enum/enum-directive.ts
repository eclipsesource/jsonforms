...
class EnumDirective implements ng.IDirective {
    ...
    template = `
    <jsonforms-control>
      <select  ng-options="option as option for option in vm.options" id="{{vm.id}}" class="form-control jsf-control jsf-control-enum" ng-change='vm.modelChanged()' ng-model="vm.modelValue[vm.fragment]"></select>
    </jsonforms-control>`;
