...
export abstract class AbstractControl implements IRuleServiceCallBack{
    ...
    protected get label(){
        let labelObject = LabelObjectUtil.getElementLabelObject(this.uiSchema.label, this.schemaPath);
    ...
    private validate() {
        ...
        var result = validationService.getResult(this.data, '/' + PathUtil.normalize(this.schemaPath));
        this.alerts = [];
        if (result !== undefined) {
            var alert = {
                type: 'danger',
...
export var ControlRendererTester=function(type:string,specificity:number):RendererTester{
    return function (element:IUISchemaElement, dataSchema:any, dataObject:any,pathResolver:IPathResolver ){
