...
When using the redux integration you'll need to use the [registerRenderer](/docs/available-actions#registerrenderertester-rankedtester-renderer-any) action:
```
import { rankWith, scopeEndsWith } from '@jsonforms/core';
import { Rating } from './Rating';
...
export withJsonFormsControlProps(RatingControl);

// Register the renderer in your store
store.dispatch(Actions.registerRenderer(rankWith(3,scopeEndsWith('rating')), RatingControl));
```
