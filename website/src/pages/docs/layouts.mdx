...
<ApiLink link={'/public/api/core/interfaces/layout.html'} title='Layout' /> elements in the UI Schema contain other
...
## HorizontalLayout 
<ApiLink link={'/public/api/core/interfaces/horizontallayout.html'} title='Horizontal Layout'/> uses
the `HorizontalLayout` type and arranges its contained `elements` in a
...
## VerticalLayout
<ApiLink link={'/public/api/core/interfaces/verticallayout.html'} title='Vertical Layout'/> uses the `VerticalLayout`
type and arranges its `elements` in a vertical fashion,  i.e. all elements are placed beneath each other.
...
## Group
<ApiLink link='/public/api/core/interfaces/grouplayout.html'>Groups</ApiLink> are very similar to
a `VerticalLayout`s but feature an additional mandatory `label` property
...
<ApiLink link='/public/api/core/interfaces/categorization.html'>Categorization</ApiLink> uses
the `Categorization` type and can only contain `elements` of
