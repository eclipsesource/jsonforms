...
The styles are simple objects with a name and an array of class names to apply. 
You can also provide a function to the `classNames` parameter, which allows you to return dynamic class names.
Here is an example:
...
The difference is that `getStyleAsClassName` returns a string whereas `getStyle` returns an array of strings.
Here is an example:
