...
<Note
  ...
  body={<span>As of JSON Forms 2.5 the React-Redux variant is deprecated in favor of the <a href="/docs/react">JSON Forms "standalone" component</a>.
        ...
        For the React standalone tutorial, see <a href="/docs/tutorial">here</a></span>}
/>
...
2. Install JSON Forms and the material renderer set. We'll use an example from JSON Forms in order to obtain
   ...
   configured as peer dependencies.
...
6. Open the `App.js` (`App.tsx` when using typescript) file and import the `JsonFormsDispatch` component from `@jsonforms/react`.
   Delete the `header` element and replace it with the `JsonFormsDispatch` element
