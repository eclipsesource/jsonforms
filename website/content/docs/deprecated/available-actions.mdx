...
JSON Forms provides a couple of actions to interact with the store, which we describe here.
All actions might either be imported directly or via the Actions utility.
...
`mapDispatchToControlProps` and `mapDispatchToFieldProps` provide a helper function called `handleChange` which already dispatches the update action, so that you rarely need to interact with update itself, if at all.
